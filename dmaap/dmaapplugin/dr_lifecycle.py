...
def get_existing_feed(**kwargs):
    ...
    try:
        ...
        feed_id_input = False
        if "feed_id" in list(ctx.node.properties.keys()):
            feed_id_input = True
            f = dmc.get_feed_info(ctx.node.properties["feed_id"])
        elif "feed_name" in list(ctx.node.properties.keys()):
